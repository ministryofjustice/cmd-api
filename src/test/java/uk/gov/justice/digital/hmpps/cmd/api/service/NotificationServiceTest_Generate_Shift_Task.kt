...
internal class NotificationServiceTest_Generate_Shift_Task {
    ...
    inner class GenerateAndSaveNotificationTests {
        ...
        fun `Should disregard Shift Task if it exists in our db`() {
            ...
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, start, any(), today.atStartOfDay()) } returns 1
        ...
        fun `Should add multiple Shift Task notifications for one user`() {
            ...
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, any(), any(), today.atStartOfDay()) } returns 0

            val results = mutableListOf<Collection<Notification>>()
            every { shiftNotificationRepository.saveAll(capture(results)) } returns listOf()

            service.refreshNotifications()
            val notification1 = Notification(null, quantumId, today.atStartOfDay(), start.plusDays(1), end, task, ShiftType.SHIFT, ShiftActionType.EDIT, false)
            val notification2 = Notification(null, quantumId, today.atStartOfDay(), start.plusDays(2), end, task, ShiftType.SHIFT, ShiftActionType.EDIT, false)
            val notification3 = Notification(null, quantumId, today.atStartOfDay(), start.plusDays(3), end, task, ShiftType.SHIFT, ShiftActionType.EDIT, false)
        ...
        fun `Should not add multiple duplicate Shift Task notifications for one user`() {
            ...
            service.refreshNotifications()
            val notification1 = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.ADD, false)
        ...
        fun `Should filter out duplicate Shift Task notifications when there is an OverTime one too`() {
            ...
            service.refreshNotifications()
            val notification1 = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.OVERTIME, ShiftActionType.EDIT, false)
            val notification2 = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.EDIT, false)
        ...
        fun `Should add multiple notifications for same shift task with different modified times for one user`() {
            ...
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, any(), any(), today.atStartOfDay()) } returns 0
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, any(), any(), today.atStartOfDay().plusSeconds(5)) } returns 0
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, any(), any(), today.atStartOfDay().plusSeconds(10)) } returns 0
            ...
            service.refreshNotifications()
            val notification1 = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.ADD, false)
            val notification2 = Notification(null, quantumId, today.atStartOfDay().plusSeconds(5), start, end, task, ShiftType.SHIFT, ShiftActionType.ADD, false)
            val notification3 = Notification(null, quantumId, today.atStartOfDay().plusSeconds(10), start, end, task, ShiftType.SHIFT, ShiftActionType.ADD, false)
            assertThat(results[0]).isEqualTo(listOf(notification1,notification2,notification3))
        }
        ...
        fun `Should save edit Shift Task notification types`() {
            ...
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, start, any(), today.atStartOfDay()) } returns 0

            val results = mutableListOf<Collection<Notification>>()
            every { shiftNotificationRepository.saveAll(capture(results)) } returns listOf()

            service.refreshNotifications()
            val expected = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.EDIT, false)
        ...
        fun `Should save Add shift Task notification types if not exist in the DB`() {
            ...
            val expected = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.ADD, false)
            assertThat(results[0]).isEqualTo(listOf(expected))
        }
        ...
        fun `Should save remove Shift Task notification types if not exist in the DB`() {
            ...
            val expected = Notification(null, quantumId, today.atStartOfDay(), start, end, task, ShiftType.SHIFT, ShiftActionType.DELETE, false)
            assertThat(results[0]).isEqualTo(listOf(expected))
        }
        ...
        fun `Should disregard unprocessed Shift Task Notification duplicates`() {
            ...
            every { shiftNotificationRepository.countAllByQuantumIdIgnoreCaseAndDetailStartAndShiftTypeAndShiftModified(quantumId, any(), ShiftType.SHIFT, today.atStartOfDay()) } returns 1
